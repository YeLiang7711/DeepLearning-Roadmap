...
### 1.1 线性代数
- 📂 [[0-概述-线性代数|概述]] — 全章速查表与推荐阅读顺序
- 📄 [[1.1.1 向量、矩阵、张量与常用运算|1.1.1 向量、矩阵、张量]] — 标量/向量/矩阵/张量、矩阵乘法、转置、Hadamard 积
- 📄 [[1.1.2 内积、范数、距离与相似度|1.1.2 内积、范数、距离]] — 点积、L1/L2/Frobenius 范数、欧氏/余弦/Mahalanobis 距离
- 📄 [[1.1.3 线性相关、秩、基与子空间|1.1.3 线性相关、秩、基]] — 线性相关与无关、秩的定义与意义、基与子空间
- 📄 [[1.1.4 特征值、特征向量、SVD|1.1.4 特征值、特征向量、SVD]] — 特征分解、奇异值分解、低秩近似
- 📄 [[1.1.5 正定矩阵、协方差矩阵、求逆与伪逆|1.1.5 正定矩阵、协方差矩阵]] — 正定/半正定、协方差矩阵、矩阵求逆/伪逆/shrinkage
- 📄 [[1.1.6 PCA及低秩近似|1.1.6 PCA 及低秩近似]] — PCA 降维/可视化/去噪/去相关/异常检测/压缩 六大应用

### 1.2 微积分
- 📂 [[0-概述-微积分|概述]] — 全章概览与阅读顺序
- 📄 [[1.2.1 导数、偏导数、方向导数与梯度|1.2.1 导数、偏导数、梯度]] — 变化率、偏导、梯度定义与梯度下降
- 📄 [[1.2.2 链式法则与反向传播|1.2.2 链式法则与反向传播]] — 复合函数求导、计算图、反向传播推导
- 📄 [[1.2.3 Jacobian、Hessian及其直觉|1.2.3 Jacobian 与 Hessian]] — 向量值函数导数、二阶导数矩阵、高维鞍点
- 📄 [[1.2.4 常见函数的导数|1.2.4 常见函数的导数]] — 激活函数/损失函数的导数速查表

### 1.3 概率统计
- 📂 [[0-概述-概率统计|概述]] — 全章概览与阅读顺序
- 📄 [[1.3.1 随机变量、期望、方差、协方差|1.3.1 随机变量、期望、方差]] — 离散/连续随机变量、期望与方差性质
- 📄 [[1.3.2 条件概率与贝叶斯公式|1.3.2 条件概率与贝叶斯]] — 条件概率、贝叶斯定理、先验/后验/似然
- 📄 [[1.3.3 高斯分布与多元高斯分布|1.3.3 高斯与多元高斯]] — 一元/多元高斯、Σ 控制形状、68-95-99.7 规则
- 📄 [[1.3.4 最大似然估计与最大后验估计|1.3.4 MLE 与 MAP]] — MLE→损失函数、MAP→L2 正则化
- 📄 [[1.3.5 置信区间、分位数、假设检验|1.3.5 置信区间与假设检验]] — 分位数阈值、两类错误、AUROC 统计含义
- 📄 [[1.3.6 稳健统计|1.3.6 稳健统计]] — Median/MAD、Huber 损失、Breakdown Point
- 📄 [[1.3.7 Mahalanobis距离与协方差收缩|1.3.7 Mahalanobis 与收缩]] — 样本协方差的统计问题、Shrinkage、Ledoit-Wolf
- 📄 [[1.3.8 偏差—方差权衡|1.3.8 偏差—方差权衡]] — 测试误差分解、过拟合/欠拟合诊断、双下降

### 1.4 优化
- 📂 [[0-概述-优化|概述]] — 全章概览与阅读顺序
- 📄 [[1.4.1 梯度下降与随机梯度下降|1.4.1 梯度下降与 SGD]] — 下山比喻、GD/SGD/Mini-batch 变体、学习率直觉
- 📄 [[1.4.2 Momentum、Adam、AdamW|1.4.2 Momentum 与 Adam]] — 惯性、自适应步长、解耦权重衰减
- 📄 [[1.4.3 学习率、学习率调度与权重衰减|1.4.3 学习率与调度]] — Step/Cosine/Warmup、权重衰减=L2
- 📄 [[1.4.4 凸优化与非凸优化|1.4.4 凸优化与非凸优化]] — 碗 vs 山脉、鞍点、局部极小
- 📄 [[1.4.5 约束与正则化（L1与L2）|1.4.5 约束与正则化]] — L1/L2 约束几何、MAP 对应

### 1.5 信息论
- 📂 [[0-概述-信息论|概述]] — 全章概览与阅读顺序
- 📄 [[1.5.1 熵|1.5.1 熵]] — 信息量、二元熵函数、不确定性度量
- 📄 [[1.5.2 交叉熵|1.5.2 交叉熵]] — 分类损失、与熵/KL 的关系
- 📄 [[1.5.3 KL散度|1.5.3 KL 散度]] — 分布差距、不对称性、蒸馏/VAE 应用
- 📄 [[1.5.4 互信息|1.5.4 互信息]] — 共享信息、文氏图、表征学习目标
- 📄 [[1.5.5 信息瓶颈|1.5.5 信息瓶颈]] — 压缩+保留、漏斗结构、泛化解释

---

## 02-Python与PyTorch

### 2.1 Python 科学计算与数据工具（路线图第 2 章）
- 📂 [[0-概述-Python科学计算与数据工具|概述]] — 全节概览与阅读顺序
- 📄 [[2.1.1 Python语法、函数、类、迭代器与上下文管理器|2.1.1 Python 语言基础]] — 语法/容器/函数/类/生成器/with
- 📄 [[2.1.2 类型标注、异常处理与日志|2.1.2 类型标注与日志]] — typing、try/except、logging
- 📄 [[2.1.3 NumPy数组、广播、索引与向量化|2.1.3 NumPy]] — 数组/广播/索引/向量化
- 📄 [[2.1.4 SciPy滤波、插值、统计与信号处理|2.1.4 SciPy]] — 滤波/插值/统计/信号处理
- 📄 [[2.1.5 pandas表格、元数据与实验结果整理|2.1.5 pandas]] — 表格/元数据/实验结果整理
- 📄 [[2.1.6 Matplotlib与Seaborn可视化|2.1.6 可视化]] — 折线/热图/直方图/箱线图
- 📄 [[2.1.7 scikit-learn统计学习与指标|2.1.7 scikit-learn]] — PCA/聚类/SVM/指标/异常基线
- 📄 [[2.1.8 经典机器学习模型SVM与聚类|2.1.8 经典机器学习]] — SVM、KMeans、DBSCAN
- 📄 [[2.1.9 OpenCV图像处理|2.1.9 OpenCV]] — 对齐/滤波/形态学/连通域
- 📄 [[2.1.10 配置、命令行、随机种子与目录规范|2.1.10 工程规范]] — 配置/YAML/argparse/种子/目录
- 📄 [[2.1.11 Git基础、环境管理与依赖锁定|2.1.11 Git 与环境]] — Git/conda/依赖锁定

### 2.2 PyTorch 训练闭环（路线图第 3 章）
- 📂 [[0-概述-PyTorch训练闭环|概述]] — 全节概览与阅读顺序
- 📄 [[2.2.1 Tensor与自动微分|2.2.1 Tensor 与自动微分]] — Tensor/requires_grad/backward/计算图
- 📄 [[2.2.2 Dataset与DataLoader|2.2.2 Dataset 与 DataLoader]] — 数据集/批量加载/shuffle/采样器/collate
- 📄 [[2.2.3 nn.Module|2.2.3 nn.Module]] — forward/参数/缓冲区/train-eval
- 📄 [[2.2.4 损失函数与优化器|2.2.4 损失与优化器]] — CrossEntropy/AdamW/调度器
- 📄 [[2.2.5 训练循环|2.2.5 训练循环]] — 6 动作循环/zero_grad/梯度裁剪/模板
- 📄 [[2.2.6 checkpoint与推理|2.2.6 checkpoint 与推理]] — state_dict/四件套/weights_only/ONNX
- 📄 [[2.2.7 GPU与混合精度|2.2.7 GPU 与混合精度]] — device/显存优化/AMP/scaler
- 📄 [[2.2.8 Hook与特征提取|2.2.8 Hook 与特征提取]] — forward/backward hook/特征可视化
- 📄 [[2.2.9 单元测试与数值稳定性|2.2.9 单元测试]] — shape 断言/梯度检查/最小过拟合测试/NaN 防护
...
- 📄 [[距离度量在异常检测中的应用|距离度量在异常检测中的应用]] — 欧氏→PatchCore / Mahalanobis→PaDiM
